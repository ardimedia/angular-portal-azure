...
export class Blade extends AzurePortal.UserControlBase {
    ...
    constructor(public portalService: AzurePortal.PortalService, path: string, title: string, subtitle: string = '', width: number = 200) {
        super(portalService);
        ...
        if (!portalService) { throw new Error('[AzurePortal.Blade] constructor parameter \'portalService\' must be provided.'); }
        if (!path) { throw new Error('[AzurePortal.Blade] constructor parameter \'path\' must be a string.'); }
        if (!title && title !== '') { throw new Error('[AzurePortal.Blade] constructor parameter \'title\' must be a string when provided.'); }
        if (!subtitle && subtitle !== '') { throw new Error('[AzurePortal.Blade] constructor parameter \'subtitle\' must be a string when provided.'); }
        if (!width && width !== 0) { throw new Error('[AzurePortal.Blade] constructor parameter \'width\' must be a number when provided.'); }

        if (width < 50) { throw new Error('[AzurePortal.Blade] constructor parameter \'width\' must be at least 50.'); }
        ...
        // Register listener1
        this.listener1 = that.portalService.$rootScope.$on('BladeArea.AddBlade', function (event, args: AzurePortal.IAddBladeEventArgs) {
