describe('AzurePortal.Blade', function () {

    //#region Tests initialize

    var sut: AzurePortal.Blade;
    ...
    beforeEach(angular.mock.inject(function ($injector) {
        ...
        sut = new AzurePortal.Blade(portalService, 'test-path', 'test-title');
    }));
    ...
    it("Constructor with parameter [width = 300] successful", function () {
        var sut = new AzurePortal.Blade(portalService, 'test-path', '', '', 300);
    ...
    it("Constructor with parameter [width => 50] throws no expection", function () {
        var sut = new AzurePortal.Blade(portalService, 'test-path', '', '', 50);

        expect(sut).not.toBe(null);
    });

    it("Constructor with parameter [width < 50] throws expection", function () {
        var sut = function () { new AzurePortal.Blade(portalService, 'test-path', '', '', 49) };
    ...
    it("navigateTo throws expection - abstract function", function () {
        ...
        expect(exception).toThrow(new Error('[AzurePortal.Blade] \'onNavigateTo\' is an abstract function. Define one in the derived class.'));
    });
    ...
    it("close throws exception - path could not be removed", function () {
        ...
        expect(exception).toThrow(new Error('[AzurePortal.Blade] path: \'test-path\' could not be removed, since no \'this.portalService.bladeArea\' available.'));
    });
