...
export class AreaBlades extends UserControlBase {
    ...
    constructor(public portalService: PortalService) { // $scope: angular.IScope
    ...
    private setupShowHideNotificationAreaListener() {
        if (this.portalService.$rootScope) {
            this.areaNotificationShowListener = this.portalService.$rootScope.$on('AreaNotification.Show', (event: any, args: IAddBladeEventArgs) => { // ng.IAngularEvent
                this.setPortalScrollCss();
            });

            this.areaNotificationHideListener = this.portalService.$rootScope.$on('AreaNotification.Hide', (event: any, args: IAddBladeEventArgs) => { // ng.IAngularEvent
                this.setPortalScrollCss();
            });
        }
    }
    ...
    private setupAddBladeListener() {
        if (this.portalService.$rootScope) {
            this.addBladeListener = this.portalService.$rootScope.$on('AreaBlades.AddBlade', (event: any, args: IAddBladeEventArgs) => { // ng.IAngularEvent
