...
export class AreaBlades extends UserControlBase {
  ...
  addBlade(path: string, senderPath: string = ''): Blade | void {
    ...
    //let blade = new Blade(this.$scope, this.portalService, path, '');
  ...
  private setPortalScrollCss() {
    ...
    if (this.portalScroll != null && this.portalService.areaNotification != null) {
      ...
      console.log('area-blades.setPortalScrollCss() not set.');
    }
  }

  //private setupShowHideNotificationAreaListener() {
  //  this.areaNotificationShowListener = this.portalService.$rootScope.$on('AreaNotification.Show', (event: ng.IAngularEvent, args: IAddBladeEventArgs) => {
  //    this.setPortalScrollCss();
  //  });

  //  this.areaNotificationHideListener = this.portalService.$rootScope.$on('AreaNotification.Hide', (event: ng.IAngularEvent, args: IAddBladeEventArgs) => {
  //    this.setPortalScrollCss();
  //  });
  //}

  //private setupAddBladeListener() {
  //  this.addBladeListener = this.portalService.$rootScope.$on('AreaBlades.AddBlade', (event: ng.IAngularEvent, args: IAddBladeEventArgs) => {
